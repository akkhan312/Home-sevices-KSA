...
export default function CustomerChatScreen() {
  const { id: bookingId, providerName: paramProviderName, shareLocation } = useLocalSearchParams<{
    id: string; providerName?: string; shareLocation?: string;
  }>();
  ...
  const rawMessages = messagesByBooking[bookingId as string] || [];
  ...
  const fetchProviderAndBooking = async () => {
    try {
      ...
      if (bRes.ok) {
        ...
        if (booking.providerId) {
          const pRes = await fetch(`${API}/providers/${booking.providerId}`);
          if (pRes.ok) {
            const pData = await pRes.json();
            let pic = pData.profile_picture || pData.profilePicture || '';
            if (pic && !pic.startsWith('http')) pic = `${API_HOST}${pic}`;

            setProviderInfo({
              id: pData.id,
              name: pData.name || booking.providerName || 'Provider',
              profilePicture: pic,
              rating: pData.rating || 0,
              city: pData.city || '',
              isVerified: Boolean(pData.verified)
            });
          }
        }
      }
  ...
  useEffect(() => {
    ...
    fetchProviderAndBooking();
    loadHistory().then(() => {
      // "Location" button on the order screen opens the chat and shares the current location.
      if (shareLocation === '1') shareCurrentLocation();
    });
    ...
    const unsubMsg = subscribe('new_message', (msg: ChatMessage) => {
      if (msg.bookingId === bookingId) {
        ...
        // Auto scroll ONLY if user is reading near bottom
    ...
    const unsubCallRejected = subscribe('call_rejected', (data: any) => {
      if (data.bookingId === bookingId) {
        ...
        Alert.alert('Call Declined', `${providerInfo?.name || 'Provider'} declined the call.`);
      }
    });
  ...
  const sendMessagePayload = async (payload: {
    ...
    const messageData: ChatMessage = {
      ...
      voiceUrl: payload.voiceUrl || undefined,
      voiceDuration: payload.voiceDuration || 0,
      documentUrl: payload.documentUrl || undefined,
      documentName: payload.documentName || undefined,
  ...
  const openProviderProfile = () => {
    if (providerInfo?.id) {
      router.push(`/(customer)/provider/${providerInfo.id}` as any);
    }
  };
  ...
  const handleVoiceSent = (voiceUrl: string, durationSec: number) => {
    sendMessagePayload({ voiceUrl, voiceDuration: durationSec, text: '🎤 Voice message' });
  };
  ...
  const renderMessageItem = ({ item }: { item: ChatMessage }) => {
    ...
    return (
      <MessageBubble
        ...
        onReplyPress={(msg) => setReplyToMessage(msg)}
  ...
  const displayName = providerInfo?.name || paramProviderName || 'Service Provider';
  const isOnline = providerInfo?.id ? (onlineUsers.has(providerInfo.id) || connected) : connected;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={[styles.headerContent, getResponsiveContainerStyle()]}>
          ...
          <TouchableOpacity onPress={openProviderProfile} style={styles.headerInfo} activeOpacity={0.8}>
            <CachedImage uri={providerInfo?.profilePicture} name={displayName} size={38} />
            ...
            <View>
              ...
              <View style={styles.statusRow}>
                ...
                {providerInfo?.rating ? (
                  <Text style={styles.ratingText}> · ⭐ {providerInfo.rating.toFixed(1)}</Text>
                ) : null}
              </View>
            </View>
          </TouchableOpacity>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <TouchableOpacity onPress={() => setShowSearch(!showSearch)} style={styles.headerActionBtn}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <View style={styles.emptyChat}>
                  ...
                  <Text style={styles.emptyChatSub}>Start instant real-time conversation!</Text>
                </View>
              }
      ...
      <AudioCallModal
        ...
        counterpartAvatar={providerInfo?.profilePicture}
...
const styles = StyleSheet.create({
  ...
  headerAvatar: { width: 38, height: 38, borderRadius: 19, backgroundColor: '#2E8B57', justifyContent: 'center', alignItems: 'center' },
  ...
  ratingText: { color: '#FCD34D', fontSize: 11, fontWeight: '700' },
  ...
  searchBarBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#0F2444', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 6, marginTop: 10, gap: 8 },
